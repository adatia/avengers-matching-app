...
//
//  Created by Period Two on 2019-05-15.
...
class TimeTrialViewController: UIViewController {
    ...
    //creates the variables for the timer
    var timer:Timer?
    var timeLeft = 60
    
    //creates outlets for the score label, the timer and the start, give up and new game buttons
    ...
    func win () {
        let alertController = UIAlertController(title: "Game Over", message:
            "Well done! Your score was \(score) and you had \(timeLeft) seconds to spare!", preferredStyle: .alert)
    ...
    //creates the function that decrements the timer
    @objc func decreaseTimer()
    {
        ...
        if timeLeft <= 0 {
            let alertController = UIAlertController(title: "Game Over", message:
                "Game over. Your score was \(score).", preferredStyle: .alert)
            alertController.addAction(UIAlertAction(title: "Continue", style: .default))
            self.present(alertController, animated: true, completion: nil)
            timer!.invalidate()
            timer = nil
    ...
    @IBAction func newGame(_ sender: UIButton) {
        ...
        //after 6.4 seconds, reveals the start button and resets the clock
    ...
    func saveData() {
        ...
        alert.addAction(UIAlertAction(title: "OK", style: .default, handler: { action in
            ...
            self.savedData2.append("Score: \(self.superScore), Player: \((alert.textFields?.first!.text)!), Time: \(60 - self.timeLeft) seconds")
